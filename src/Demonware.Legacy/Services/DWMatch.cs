...
namespace DWServer
{
    public class DWMatch
    {
                ...
                try
                {
                    switch (call)
                    {
                        ...
                        case 14:
                            ...
                            break;
                        case 16:
                            FindSessionsTwoPass(data, packet);
                            break;
                        default:
        ...
        /// <summary>
        /// helpDW bdMatchMaking::find_sessions_two_pass (task 16) — return all known sessions.
        /// </summary>
        private static void FindSessionsTwoPass(MessageData mdata, DWMessage packet)
        {
            IEnumerable<KeyValuePair<ulong, bdMatchMakingInfo>> matchingSessions;
            ...
            lock (_sessions)
            {
                // helpDW returns every known session (no title filter).
                matchingSessions = _sessions.ToList();
            ...
            reply.Send(true);
            Console.WriteLine("find_sessions_two_pass returned {0} sessions", matchingSessions.Count());
        ...
        private static void FindSessions(MessageData mdata, DWMessage packet)
        {
            ...
            {
                // IW6 / Ghosts: return all sessions (same as find_sessions_two_pass)
                FindSessionsTwoPass(mdata, packet);
            }
        }
        ...
        private static void CreateSession(MessageData mdata, DWMessage packet)
        {
            ...
            {
                // IW6 / Ghosts / unknown — base session payload
                info = new MatchMakingInfoIW6();
            }
            ...
            {
                Log.Debug("Registered session (" + titleID + ").");
            }
        }
    ...
    /// <summary>
    /// Minimal IW6/Ghosts session info — uses base serialize/deserialize fields.
    /// </summary>
    public class MatchMakingInfoIW6 : bdMatchMakingInfo
    {
        public override void Deserialize(DWMessage packet)
        {
            base.Deserialize(packet);
    ...
    public class bdMatchMakingInfo
    {
        ...
        public TitleID TitleID
        {
            get
            {
                ...
                else if (this is MatchMakingInfoIW6)
                {
                    return TitleID.IW6;
                }
        ...
        public string CountryCode
        {
            get
            {
                try
                {
                    if (DWDML._geoIP != null)
                    {
                        var loc = DWDML._geoIP.getLocation(someIP);
                        if (loc != null && !string.IsNullOrEmpty(loc.countryCode))
